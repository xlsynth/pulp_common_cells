...
module isochronous_4phase_handshake (
  ...
  `FFLSRN(src_req_q, ~src_req_q, (src_valid_i && src_ready_o), 1'b0, src_clk_i, src_rst_ni)
  // "synchronize" the acknowledge into the sending clock-domain
  `FFSRN(src_ack_q, dst_ack_q, 1'b0, src_clk_i, src_rst_ni)
  ...
  `FFLSRN(dst_ack_q, ~dst_ack_q, (dst_valid_o && dst_ready_i), 1'b0, dst_clk_i, dst_rst_ni)
  // "synchronize" the request into the receiving clock domain
  `FFSRN(dst_req_q, src_req_q, 1'b0, dst_clk_i, dst_rst_ni)
